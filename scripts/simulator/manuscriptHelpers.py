...
def moveTo(r, x, y, phi):moveTo(r, Pose(x,y,phi))}

def moveTo(r, via, to):moveTo(r,via)moveTo(r,to)}

def moveTo(r, via1, via2, to):moveTo(r,via1)moveTo(r,via2)moveTo(r,to)}

def moveTo(r, via1, via2, via3, to):moveTo(r,via1)moveTo(r,via2)moveTo(r,via3)moveTo(r,to)}

def moveTo(r, x1, y1, phi1,
                     x2, y2, phi2,
                     x, y, phi)
  :moveTo(r, Pose(x1,y1,phi1))moveTo(r, Pose(x2,y2,phi2))moveTo(r, Pose(x,y,phi))}

def moveTo(r, x1, y1, phi1,
                     x, y, phi)
  :moveTo(r, Pose(x1,y1,phi1))moveTo(r, Pose(x,y,phi))}
...
def moveToBacking(r,  startTime, to):
  ...
  mostRecentRecordedPose[indexOf(r)]=to
  # arbitrary speed in this version
  for(int i=0i<route.noOfStepsi++) Event(startTime+float(i)/50,r,route.poses[i])
}
...
def moveToBacking(r, x, y, phi):
    moveToBacking(r, Pose(x,y,phi))

def moveToBacking(r, via, to):
      moveToBacking(r,via)moveTo(r,to)
...
def moveToBacking(r, x1, y1, phi1,
                     x2, y2, phi2,
                     x, y, phi):
    moveToBacking(r, Pose(x1,y1,phi1))
    moveToBacking(r, Pose(x2,y2,phi2))
    moveToBacking(r, Pose(x,y,phi))
