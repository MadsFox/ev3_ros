...
class Route {  // find bedre navn
  ...
  float motorSpeedDiff;

  Route(int n, Pose[] pp, float td) {
    noOfSteps=n; 
    poses=pp;
    travelDist=td;
  }

  Route(int n, Pose[] pp, float td, float msd) {
    noOfSteps=n; 
    poses=pp;
    motorSpeedDiff=msd;
    travelDist=td;
...
Route forwardRoute(Pose from, Pose to) {
  float travelDist = 0;
  ArrayList<Pose> firstHalf = new ArrayList<Pose>();
  ArrayList<Pose> lastHalf = new ArrayList<Pose>();

  // catch infinite loops:
  int pathLength=1;

  Pose currentFrom=from; 
  Pose currentTo=to;
  /*if ((from.position.x == to.position.x && from.direction == to.direction) || 
    (from.position.y == to.position.y && from.direction == to.direction)) {
    while (dist(currentFrom, currentTo)>=2*epsilon && pathLength<100000) {
      println("driving straight");
      pathLength+=2;
      Pose nextForward = currentFrom.klone();
      nextForward.moveRelPhiD(0, epsilon);
      firstHalf.add(nextForward); 
      travelDist+=epsilon; 
      currentFrom=nextForward;
      lastHalf.add(currentTo);
      Pose nextBackward = currentTo.klone();
      nextBackward.moveRelPhiD(0, -epsilon); // break symmetry by change of direction
      currentTo=nextBackward; 
      travelDist+=epsilon;
    }
  } else {*/
    while (dist(currentFrom, currentTo)>=2*epsilon && pathLength<100000) {
      ...
      for (int k=0; k<=100; k++) {  // use odd number (100+1) so "straight ahead" is also an option
        Pose suggest = currentFrom.klone();
        suggest.moveRelPhiD(-maxTurn+float(k)/100*2*maxTurn, epsilon);
      ...
      for (int k=0; k<=100; k++) {  // use odd number (100+1) so "straight ahead" is also an option
        ...
        suggest.moveRelPhiD(-maxTurn+float(k)/100*2*maxTurn, -epsilon); // break symmetry by change of direction
  ...
  allPoses[firstHalf.size()] = avg(currentFrom, currentTo); 
  travelDist=dist(from, to);
...
Route mkBackwardRouteFromBotheEndsINTERNAL(Pose from, Pose to) {
  ...
  // reverse
  int m= r.noOfSteps/2; 
  if (r.noOfSteps/2*2==r.noOfSteps) m+=1; 
  for (int i=0; i<m; i++) {
    Pose p=r.poses[i];
